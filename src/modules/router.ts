...
export function addComponentToRoutes(routes: RouteRaw[]): RouteRecordRaw[] {
  return routes.map((route) => ({
    ...
    children: route.children
      ? addComponentToRoutes(route.children as RouteRaw[])
      : [],
...
export default defineNuxtModule<ModuleOptions>({
  ...
  hooks: {
    "build:before": function () {
      ...
      const nuxt = useNuxt();
      const options = defu(nuxt.options.routerModule || {}, {
        ...DEFAULTS,
      ...
      addTemplate({
        ...
        getContents: async () => {
          const { createRoutes } = await import(
            options.fileName /** vite-ignore */
          );
          const pages = createRoutes().map((route: RouteRaw) => {
            return { ...route, file: route.component };
          });
          await nuxt.callHook("pages:extend", pages);
          const serializedRoutes: Partial<RouteRecordRaw>[] =
            addComponentToRoutes(pages);
  ...
  async setup(options, nuxt) {
    const routerFilePath = await resolvePath(
    ...
    nuxt.hook("builder:watch", async (event, path) => {
      const pathPattern = new RegExp(
        `^(${routerFilePath}|${nuxt.options.srcDir}/views)/`
      );
