...
export interface PrimeVueOptions extends PrimeVueConfig {
  components?: {
    ...
    include?: Array<
      PrimevueComponents | { name: PrimevueComponents; global?: boolean }
    >;
    global?: boolean;
  };
}
...
async function registerComponent(name: PrimevueComponents, global = true) {
  await addComponent({
    ...
    global:
      ["ConfirmDialog", "ConfirmPopup", "Toast", "Tooltip"].includes(name) ||
      global,
    name,
  });
}
  ...
  async setup(options, nuxt) {
    ...
    if (options.components?.include) {
      for (const component of options.components?.include) {
        if (isObject(component)) {
          await registerComponent(
            component.name,
            component.global || options.components.global
          );
        } else {
      ...
      for (const component of primevueComponents) {
        if (
          !isArray(options.components?.exclude) ||
          !options.components?.exclude.includes(component)
        ) {
          await registerComponent(component, options.components?.global);
        }
      }
